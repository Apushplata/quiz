...
function gameOver() {
   timerDisplay = '';
    ...
    // Display the answer key and user's answers
    var answerKey = document.createElement("div");
    for (var i = 0; i < theQuestions.length; i++) {
        var questionAnswer = document.createElement("p");
        questionAnswer.textContent = theQuestions[i].question + " - Correct Answer: " + theQuestions[i].answer + " - Your Answer: " + userAnswers[i];
        answerKey.appendChild(questionAnswer);
    }
    gameOverScreen.appendChild(answerKey);
